...
import { bls, CurveFn } from './abstract/bls';
...
export const Fp = mod.Field(Fp_raw);
type Fp = bigint;
// Finite field over r.
...
export const Fp2: mod.IField<Fp2> & Fp2Utils = {
  ORDER: FP2_ORDER,
...
export const Fp12Multiply = ({ c0, c1 }: Fp12, rhs: Fp12 | bigint) => {
...
export const Fp12_finalExponentiate = (num: Fp12) => {
  const x = BLS_X;
  // this^(q⁶) / this
...
export const Fp12: mod.IField<Fp12> & Fp12Utils = {
  ...
  // https://eprint.iacr.org/2009/565.pdf
  finalExponentiate: Fp12_finalExponentiate,
};
...
export const G1_formBytes = (bytes: Uint8Array): AffinePoint<Fp> => {
...
export const G1_mapToCurve = (scalars: bigint[]) => {
  const { x, y } = G1_SWU(Fp.create(scalars[0]));
  return isogenyMapG1(x, y);
};

export const G2_mapToCurve = (scalars: bigint[]) => {
...
export const bls12_381: CurveFn<Fp, Fp2, Fp6, Fp12> = bls({
  ...
  G1: {
    ...
    mapToCurve: G1_mapToCurve,
    fromBytes: G1_formBytes,
    toBytes: (c: ProjConstructor<bigint>, point: ProjPointType<bigint>, isCompressed: boolean ) => {
