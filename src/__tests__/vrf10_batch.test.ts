...
import { derive_vrf10_public_key, IBatchItem, VrfBatchProof10, VrfBatchVerifier } from "../vrf";
    ...
    describe("rust impl tests", () => {
        ...
        for( let i = 0; i < n; i++ )
        {
            ...
            test(`example ${i}`, () => {
                ...
                const generated_proof = VrfBatchProof10.generate( sk, pk, alpha );

                expect( generated_proof.toHash() ).toEqual( expected_proof_hash );

                // expect( VrfBatchProof10.verify( pk, alpha, generated_proof ) ).toBe( true );

                // using sk just to get some bytes that are not pk
                expect( VrfBatchProof10.verify( sk, alpha, generated_proof ) ).toBe( false );

                const wrong_alpha = fromHex( "01" + _alpha.slice(alpha[0] ?? 0) );
                expect( VrfBatchProof10.verify( pk, wrong_alpha, generated_proof ) ).toBe( false );
            });
        }
        ...
        const batchVerifier = new VrfBatchVerifier( nProofs );
        const elems: IBatchItem[] = [];
        for( let i = 0; i < nProofs; i++ )
        {
            const sk = getRandomValues( new Uint8Array( 32 ) );
        ...
        {
            test("batch verification elem", () => {
                expect( VrfBatchProof10.verify( elem.public_key, elem.alpha, elem.proof ) ).toBe( true )
                expect( VrfBatchProof10.verify( elem.alpha, elem.alpha, elem.proof ) ).toBe( false )
            });
        }

        test("batch verification", () => {
            expect( batchVerifier.verify() ).toBe( true );
        });
